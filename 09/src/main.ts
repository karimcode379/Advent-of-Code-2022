...
function HeadTailMoving(input: string[][]) {
	let hx = 0;
	let hy = 0;
	let tx = 0;
	let ty = 0;
...
function headmovement(direction: string, currentHeadPosition: number[]) {
...
function knotMovement(
	currentFirstKnotPosition: number[],
	currentSecondKnotPosition: number[]
) {
...
function getTheRope(input: string[][]) {
	...
	for (let move of input) {
		...
		for (let i = 0; i < steps; i++) {
			...
			for (let j = 0; j < positionArray.length - 1; j++) {
				let currentKnotPosition = knotMovement(
					positionArray[j],
					positionArray[j + 1]
				);
